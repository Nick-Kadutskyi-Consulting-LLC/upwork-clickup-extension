<template>
  <h5>List to save Upwork Leads to</h5>
  <div class="mb-3" v-if="selectedList">
    <label for="clApiToken" class="form-label">Current List</label>
    <input
  ...
  <div class="mb-3" v-if="teams.length > 0">
    <label for="clickUpteamSelect" class="form-label">ClickUp Team</label>
  ...
  <div class="mb-3" v-if="spaces.length > 0">
    <label for="clickUpSpaceSelect" class="form-label">ClickUp Space</label>
  ...
  <div class="mb-3" v-if="folders.length > 0">
    <label for="clickUpFolderSelect" class="form-label">ClickUp Folder</label>
  ...
  <div v-if="folderlessLists.length > 0" class="mb-3">
    <label class="form-label">Folderless Lists in <strong>{{ selectedSpace?.name }}</strong> space</label>
    <ul class="list-group">
      <li
  ...
  <div v-if="lists.length > 0" class="mb-3">
    <label class="form-label">Lists in <strong>{{ selectedFolder?.name }}</strong> folder</label>
    <ul class="list-group">
      <li
          class="list-group-item d-flex justify-content-between align-items-center"
          :class="{'list-group-item-primary': selectedList?.id === list?.id }"
